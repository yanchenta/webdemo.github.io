...
function shareToBlog(type,chann) {
	var shareContent = encodeURIComponent($('#wrapp ul li').eq(myScroll2.currPageX).text()+'http://m.kfc.com.cn/lunch');
	
	// var shareContent = encodeURIComponent("午餐时间到，肚子咕咕叫，KFC餐厅报道！15元起豪华午餐，米饭加鲜汤，汉堡搭薯条，选择多多，有滋有味，吃饱吃好又不贵！http://kfclunch.qq.com/");
	//alert(shareContent);
	if (type == "sina") {
		
		track(channel,4188,"http://newcms.mobile1.com.cn/callback/sinacallback?content="+shareContent+"&ad=kfcl&k=kfcl_"+chann+"_sina");
		
		
	} else if (type == "renren") {
		
		track(channel,4189,"http://newcms.mobile1.com.cn/callback/renrencallback?content="+shareContent+"&ad=kfcl&k=kfcl_"+chann+"_renren");
		 
		 
	} else if (type == "tecent") {
		
		track(channel,4190,"http://newcms.mobile1.com.cn/callback/qqcallback?content="+shareContent+"&ad=kfcl&k=kfcl_"+chann+"_qq");
