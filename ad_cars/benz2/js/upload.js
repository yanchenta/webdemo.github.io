	...
	adwoImgUpload.prototype.init=function(func,obj){
		...
		window.ImgControl.adwoImgUpload.uploader.addEventListener("change",function(){
			...
			img_loc.onload = function(){
				//加载后的回调
				var _canvasTrans = document.getElementsByClassName("canvas-tranform")[0];
				var _imgData = this;
				var _maskDiv = document.getElementsByClassName("img-transfrom-Layer")[0];
				...
				var _saveTransformBtn = document.getElementsByClassName("btn-save-transfrom")[0];
...
function uploadFinalImg(data){
	//var url  = 'http://newcms.mobile1.com.cn/advmessage/advimage/saveImgJsonP.action?advid=20043';
	var _url  = 'http://test.zhangkuo.net/advmessage/advimage/saveImgJsonP.action?advid=20043';
	$.ajax({
		type:'POST',
		url:_url,
		cache: false,
