...
*/
	...
	ext_upload_pieces.prototype.resetPara = function(){//重置参数
		...
		var uaFoo = ua;
		if(ua.indexOf("iphone") >= 0 || uaFoo.indexOf("iphone") >= 0){//iphone
			ext_upload_pieces.prototype.transPara.lastScale = 1;//上一次的缩放操作周期结束的"缩放参数"
			ext_upload_pieces.prototype.transPara.curScale = 1;// => 初始写为0.3
		}
		else{
			ext_upload_pieces.prototype.transPara.lastScale = 1;//上一次的缩放操作周期结束的"缩放参数"
	...
	*/
	ext_upload_pieces.prototype.detectVerticalSquash = function (img, ih) {
		...
		var ddd = ctx.getImageData(0, 0, 1, ih);
		var data = ddd.data;
	...
	ext_upload_pieces.prototype.verScale = 1;
	
	ext_upload_pieces.prototype.setImage = function(_canvas, _image, _transX, _transY, _scale, _angle, doSave){
		var ua = window.navigator.userAgent.toLowerCase();
		var uaFoo = ua;
		if(ua.indexOf("iphone") >= 0 || uaFoo.indexOf("iphone") >= 0){//iphone
			ext_upload_pieces.prototype.setImage3(_canvas, _image, _transX, _transY, _scale, _angle, doSave);
			//ext_upload_pieces.prototype.setImage2(_canvas, _image, _transX, _transY, _scale, _angle, doSave);
		}
		else{//android
			ext_upload_pieces.prototype.setImage3(_canvas, _image, _transX, _transY, _scale, _angle, doSave);
		}
	...
	ext_upload_pieces.prototype.setImage2 = function(_canvas, _image, _transX, _transY, _scale, _angle, doSave){
		...
		if(ua.indexOf("iphone") >= 0 || uaFoo.indexOf("iphone") >= 0){//不是截图图片
			if(0 === _image.offsetWidth % 320 || 0 === _image.offsetHeight % 320){
				ext_upload_pieces.prototype._ctx.scale(_new_scale, _new_scale);
			}
			else if(_angle%Math.PI > -0.01 && _angle%Math.PI < 0.01){ //0/180
				if(_image.offsetWidth <= 320 && _image.offsetHeight <= 320){
					ext_upload_pieces.prototype._ctx.scale(_new_scale, _new_scale);
				}
				else{
					if(_image.offsetWidth < _image.offsetHeight && _image.offsetWidth > 320){
						ext_upload_pieces.prototype._ctx.scale(_new_scale * 0.337, _new_scale);
					}
					else{
						ext_upload_pieces.prototype._ctx.scale(_new_scale * 0.25, _new_scale);
					}
				}
			}
			else{//90/270
				if(_image.offsetWidth <= 320 && _image.offsetHeight <= 320){
					...
					if(_image.offsetWidth < _image.offsetHeight && _image.offsetWidth > 320){
						ext_upload_pieces.prototype._ctx.scale(_new_scale, _new_scale * 0.337);
					}
					else{
						ext_upload_pieces.prototype._ctx.scale(_new_scale, _new_scale * 0.25);
					}
				}
			}
	...
	ext_upload_pieces.prototype.setImage3 = function(_canvas, _image, _transX, _transY, _scale, _angle, doSave){
		alert("(" + _canvas.width + "," + _canvas.height + "), (" + _image.width + "," + _image.height + "), transX:" + _transX + ", tranY:" + _transY + ", scale:" + _scale + ", angle:" + _angle);
		...
		*/
		...
		ext_upload_pieces.prototype._ctx.drawImage(_image, 0, 0,192,256,0,0,192,256);
