...
;(function($){
	...
	*/
	$.fn.ylmap = function(options){
		...
		// 默认参数
		$.fn.ylmap.defaults = {
			detal			: {sign_name:'',contact_tel:'',address:'天安门'},					//地址值
			latitude		: 39.915,														//纬度
		...
		return this.each(function(){
			...
			*/
			var mapBox = $('<div id="BDMap" class="BDMap"></div>');			//地图容器Jquery对象
				...
				mapBoxPaent.append($('<div class="tit"><p><a href="javascript:void(0)">'+mapVal.address+'</a></p><a class="close_map" href="javascript:void(0)"></a></div>'));
			...
			*/
			...
			mapAddInfo = function(marker_,data) {
				...
				*/
				if(typeof(data.contact_tel) == 'undefined') data.contact_tel ='';
			...
			searchComplete = function(result) {
				if (result.getNumPlans() == 0) {
					...
					if(!$('.transitBtn').length>0){
						$('#transit_result').after($('<p class="transitBtn closebtn" onclick="transit_result_close()"><a href="javascript:void(0)">关闭</a></p>'));
				...
				if($("#transit_result").hasClass("open")){
					$(".closebtn").find("a").html("关闭");
				}
				else{
					$(".closebtn").find("a").html("打开");
				}
			...
			*/
			...
			function loadfunction() {  
				...
				var style_map =
					".ylmap.open {height:92%;width:100%;}"+
					".ylmap .tit { position:absolute; left:0; bottom:0; height:42px; width:100%; overflow: hidden; background:rgba(0,0,0,0.5); }"+
					".ylmap .tit p { margin-right:100px; }"+
					".ylmap .tit p a { display:block; font-size:16px; color:#fff; height:42px; line-height:42px; padding-left:70px; background:url(/template/13/images/icon01_page.png) 15px center no-repeat; }"+
					".ylmap .tit .close_map { display:none; position: absolute; bottom: 10px; right: 10px; width: 42px; height: 42px; }"+
					".ylmap .map_close_btn{position:fixed;top:10px;left:10px;display:none;width:60px;box-shadow:0 0 2px rgba(0,0,0,0.6) inset, 0 0 2px rgba(0,0,0,0.6);height:60px;border-radius:80%;color:#fff;background:rgba(230,45,36,0.8);text-align:center;line-height:60px;font-size:16px; font-weight:bold;curson:pointer;}"+
					".ylmap.open .map_close_btn{display:block;}"+
					".ylmap.mapOpen .map_close_btn{display:block;}"+
					"#BDMap {width:100%;height:100%;}"+
					"#BDMap img{width:auto;height:auto;max-width:none!important;}"+
					".transitBtn.hide {display:none;}"+
					".transitBtn {display:block;position:fixed; z-index:1500;}"+
					".transitBtn a{display:block;width:60px;box-shadow:0 0 2px rgba(0,0,0,0.6) inset, 0 0 2px rgba(0,0,0,0.6);height:60px;border-radius:80%;color:#fff;background:rgba(230,45,36,0.8);text-align:center;line-height:60px;font-size:16px; font-weight:bold}"+
					".transitBtn.closebtn{top:10px;right:10px;}"+
					".transitBtn.bus{top:10px;right:80px;}"+
					".transitBtn.car{top:80px;right:10px;}"+
					...
					"#transit_result h1{font-size:16px!important;}"+
					"#transit_result div[onclick^='Instance']{background:none!important;}"+
					"#transit_result span{display:inline-block;font-size:12px;padding:0 5px;}"+
					"#transit_result table {font-size:16px!important;}"+
					"#transit_result table td{padding:5px 10px!important;line-height:150%!important;}"+
					".infoWindow p{margin-bottom:10px;}"+
					".infoWindow .window_btn .open_navigate{display:inline-block;margin-right:10px;border:1px solid #ccc;border-radius:6px;text-align:center;cursor:pointer;}"+
